...
pgupdateSQLTable <- function(table_name, data_df, database_datatypes, db_type = "PostgreSQL", append = TRUE, overwrite = FALSE){
    ...
    db_con <- dbConnect(driver, dbname = config$DB_NAME, 
                        host = config$DB_HOST,
                        port = 5432, user = config$DB_USER, password = Sys.getenv("DB_PASSWORD"),
                        options="-c search_path=schema1")
    
    # Getting the list of datatypes for writing to database
    varTypeDf <- database_datatypes %>% filter(table_name == table_name)
